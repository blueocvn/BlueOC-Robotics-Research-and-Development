...
CallbackReturn FeetechHardwareInterface::configure_joints_(const JointIdConfigMap& yaml_by_id) {
  ...
  joint_speeds_.assign(info_.joints.size(), 2400);  // default move speed (steps/s)
  ...
  for (size_t i = 0; i < info_.joints.size(); ++i) {
    ...
    // Optional per-joint runtime move speed (steps/s). Not an EEPROM register —
    // consumed in write(). Lets e.g. the gripper move slower than the arm.
    if (auto sp = merged_params.find("moving_speed"); sp != merged_params.end()) {
      joint_speeds_[i] = std::stoi(sp->second);
    }
...
hardware_interface::return_type FeetechHardwareInterface::write(const rclcpp::Time& /* time */,
  ...
  for (uint i = 0; i < info_.joints.size(); i++) {
    ...
    if (!info_.joints[i].command_interfaces.empty()) {
      ...
      commanded_speeds.push_back(joint_speeds_[i]);  // per-joint (moving_speed), default 2400
